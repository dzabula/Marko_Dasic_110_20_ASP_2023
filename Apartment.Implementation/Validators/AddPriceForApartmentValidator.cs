    ...
    public class AddPriceForApartmentValidator : AbstractValidator<PriceDto>
    {
        public AddPriceForApartmentValidator(ApartmentContext context)
        {
            ...
            RuleFor(x => x.ApartmentId)
                ...
                .Must(x => context.Apartments.Any(y => y.Id == x)).WithMessage("Apartman sa Id: {PropertyValue} ne postoji");
            ...
            RuleFor(x => x.PricePerNightWeekend)
               .Cascade(CascadeMode.Stop)
               .Must(x => x > 0).WithMessage("Vrednost cene za noc vikendom nije ispravna. Vrednost mora biti veca od 0");
